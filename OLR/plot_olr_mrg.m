...
%% 文件路径
olr_ncfile = 'E:\Results_of_Freddy_ChenYong_version2\olr_daily\olr_anomaly_for_MRG_daily_20220101_20231231.nc';
...
%% 读取OLR异常数据
% 注意：olr_anomaly_mrg的维度是 [lat, lon, time]
olr_data = ncread(olr_ncfile, 'olr_anomaly_mrg');
...
for i = 1:length(target_dates)
    ...
    % 标题
    title(sprintf('MRG-filtered OLR Anomaly\n%s', ...
    ...
    filename = fullfile(save_dir, sprintf('OLR_anomaly_MRG_%s.png', ...
...
if ~isempty(target_dates)
    ...
    title(sprintf('MRG-filtered Daily OLR Anomaly\n%s', ...
