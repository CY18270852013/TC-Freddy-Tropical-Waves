...
%% 文件路径
olr_ncfile = 'E:\Results_of_Freddy_ChenYong_version2\olr_daily\olr_anomaly_for_MJO_daily_20220101_20231231.nc';
...
%% 读取OLR异常数据
% 注意：olr_anomaly_mjo的维度是 [lat, lon, time]
olr_data = ncread(olr_ncfile, 'olr_anomaly_mjo');
...
for i = 1:length(target_dates)
    ...
    % 标题
    title(sprintf('MJO-filtered OLR Anomaly\n%s', ...
    ...
    filename = fullfile(save_dir, sprintf('OLR_anomaly_MJO_%s.png', ...
...
if ~isempty(target_dates)
    ...
    title(sprintf('MJO-filtered Daily OLR Anomaly\n%s', ...
