...
%% 文件路径
wind_ncfile = 'E:\Results_of_Freddy_ChenYong_version2\u_v_daily\u_v_anomaly_for_ER_daily_20220101_20231231.nc';
precip_ncfile = 'E:\Results_of_Freddy_ChenYong_version2\GPM_anomaly_daily\GPM_anomaly_for_ER_daily_20220901_20230630.nc';
...
er_data = ncread(precip_ncfile, 'er');

% 注意：er_data的维度是 [经度, 纬度, 时间]
% 需要转换为 [纬度, 经度, 时间] 以便绘图
er_data = permute(er_data, [2, 1, 3]);  % 现在维度是: lat x lon x time

% 处理缺失值
fill_value = -9999.9004;
er_data(er_data == fill_value) = NaN;
...
for i = 1:length(time_indices_wind)
    ...
    u_anom = ncread(wind_ncfile, 'u_anomaly_er', start, count);
    v_anom = ncread(wind_ncfile, 'v_anomaly_er', start, count);
...
for i = 1:length(time_indices_precip)
    ...
    rain_day = squeeze(er_data(lat_idx_precip, lon_idx_precip, day_idx));
...
for i = 1:length(target_dates)
    ...
    rain_day = squeeze(er_data(lat_idx_precip, lon_idx_precip, precip_idx));
    ...
    % 标题
    title(sprintf('ER-filtered 850 hPa Wind Anomalies & Precipitation Anomalies\n%s', ...
    ...
    % ========== 保存图形 ==========
    filename = sprintf('ER_precip_wind_anomalies_%s.png', ...
