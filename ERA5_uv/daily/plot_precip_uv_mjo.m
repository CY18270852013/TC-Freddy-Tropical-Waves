...
%% 文件路径
wind_ncfile = 'E:\Results_of_Freddy_ChenYong_version2\u_v_daily\u_v_anomaly_for_MJO_daily_20220101_20231231.nc';
precip_ncfile = 'E:\Results_of_Freddy_ChenYong_version2\GPM_anomaly_daily\GPM_anomaly_for_MJO_daily_20220901_20230630.nc';
...
mjo_data = ncread(precip_ncfile, 'mjo');

% 注意：mjo_data的维度是 [经度, 纬度, 时间]
% 需要转换为 [纬度, 经度, 时间] 以便绘图
mjo_data = permute(mjo_data, [2, 1, 3]);  % 现在维度是: lat x lon x time
...
for i = 1:length(time_indices_wind)
    ...
    u_anom = ncread(wind_ncfile, 'u_anomaly_mjo', start, count);
    v_anom = ncread(wind_ncfile, 'v_anomaly_mjo', start, count);
...
for i = 1:length(time_indices_precip)
    ...
    rain_day = squeeze(mjo_data(lat_idx_precip, lon_idx_precip, day_idx));
...
for i = 1:length(target_dates)
    ...
    rain_day = squeeze(mjo_data(lat_idx_precip, lon_idx_precip, precip_idx));
    ...
    % 标题
    title(sprintf('MJO-filtered 850 hPa Wind Anomalies & Precipitation Anomalies\n%s', ...
    ...
    % ========== 保存图形 ==========
    filename = sprintf('MJO_precip_wind_anomalies_%s.png', ...
