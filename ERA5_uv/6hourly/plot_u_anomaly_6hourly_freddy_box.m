...
%% 文件路径设置
data_path = 'E:\Results_of_Freddy\u_v_anomaly_6hourly\';

files = {
    fullfile(data_path, 'u_v_anomaly_6hourly_20220101_20231231.nc');          % 原始风场异常
    fullfile(data_path, 'u_v_anomaly_for_MJO_6hourly_20220101_20231231.nc');  % MJO滤波
    fullfile(data_path, 'u_v_anomaly_for_Kelvin_6hourly_20220101_20231231.nc'); % Kelvin滤波
    fullfile(data_path, 'u_v_anomaly_for_MRG_6hourly_20220101_20231231.nc');   % MRG滤波
    fullfile(data_path, 'u_v_anomaly_for_ER_6hourly_20220101_20231231.nc');    % ER滤波
};
...
% 处理填充值 (原始数据没有明确指定，假设NaN是填充值)
u_all(isnan(u_all)) = NaN;
...
for i = 1:length(wave_names)
    ...
    u_wave_all(u_wave_all == fill_value_wave) = NaN;
...
%% 保存图片
saveas(gcf, 'Freddy_Genesis_U_Wind_Anomaly_Contributions_6hourly.png');
fprintf('\n图片已保存为 Freddy_Genesis_U_Wind_Anomaly_Contributions_6hourly.png\n');
...
%% 保存数据到MAT文件
save('Freddy_U_Wind_Data_6hourly.mat', 'time_plot', 'u_region_mean', ...
    'u_waves', 'u_sum', 'wave_names', 'lon_range', 'lat_range', ...
    'lon_all', 'lat_all', 'time_dates');
fprintf('\n数据已保存到 Freddy_U_Wind_Data_6hourly.mat\n');
...
saveas(gcf, 'U_Wind_Contributions_Subplots_6hourly.png');
fprintf('补充图已保存为 U_Wind_Contributions_Subplots_6hourly.png\n');
...
if total_mean > 0
    ...
    title('Relative Contribution of Different Waves to U-Wind Anomaly', ...
        'FontSize', 13, 'FontWeight', 'bold');
    set(gca, 'XTick', 1:length(wave_names), 'XTickLabel', wave_names, 'FontSize', 11);
    grid on;
    ylim([0, max(wave_percentages) * 1.2]);
    
    saveas(gcf, 'U_Wind_Wave_Contribution_Percentages_6hourly.png');
    fprintf('补充图已保存为 U_Wind_Wave_Contribution_Percentages_6hourly.png\n');
else
