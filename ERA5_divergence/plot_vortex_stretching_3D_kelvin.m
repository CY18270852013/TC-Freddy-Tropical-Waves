%% 绘制Freddy生成期间CCKW对涡旋拉伸贡献的三维立体图
% 使用三维曲面展示涡旋拉伸项的空间分布
clear; clc; close all;

%% 1. 设置路径和参数
% 数据路径
kelvin_div_file = 'E:\Results_of_Freddy\divergence_anomaly_6hourly\divergence_anomaly_for_Kelvin_6hourly_20220101_20231231.nc';
era5_files = {
...
% 输出设置
output_dir = 'cckw_vortex_stretching_3d';
...
% 读取CCKW散度数据
kelvin_time_raw = ncread(kelvin_div_file, 'time'); % days since 0000-01-01
kelvin_lat = ncread(kelvin_div_file, 'latitude');
kelvin_lon = ncread(kelvin_div_file, 'longitude');
kelvin_div = ncread(kelvin_div_file, 'divergence_anomaly_kelvin'); % s^-1

% 转换时间
base_date2 = datetime(0, 1, 1, 'TimeZone', 'UTC');
kelvin_time = base_date2 + days(kelvin_time_raw) - days(1);

fprintf('CCKW数据维度: 时间=%d, 纬度=%d, 经度=%d\n', ...
    length(kelvin_time), length(kelvin_lat), length(kelvin_lon));

% 同样处理CCKW数据的网格
kelvin_lat_sorted = flipud(kelvin_lat); % 从-30到10

% 调整经度格式并排序
kelvin_lon_adj = kelvin_lon;
kelvin_lon_adj(kelvin_lon_adj > 180) = kelvin_lon_adj(kelvin_lon_adj > 180) - 360;
[kelvin_lon_sorted, kelvin_lon_sort_idx] = sort(kelvin_lon_adj);

% 重新排列kelvin_div数据
kelvin_div_sorted = zeros(size(kelvin_div));
for t = 1:size(kelvin_div, 3)
    temp = flip(kelvin_div(:, :, t), 2); % 反转纬度维度
    kelvin_div_sorted(:, :, t) = temp(kelvin_lon_sort_idx, :);
end
...
for t = 1:length(target_times)
    ...
    % 从CCKW数据中找到最接近的时间索引
    [~, kelvin_time_idx] = min(abs(kelvin_time - target_time));
    kelvin_actual_time = kelvin_time(kelvin_time_idx);
    
    fprintf('  ERA5实际时间: %s (差%.1f小时)\n', datestr(era5_actual_time), ...
        hours(abs(era5_actual_time - target_time)));
    fprintf('  CCKW实际时间: %s (差%.1f小时)\n', datestr(kelvin_actual_time), ...
        hours(abs(kelvin_actual_time - target_time)));
    
    % 提取850 hPa相对涡度 (已排序)
    vo_850_slice = squeeze(vo_850_sorted(:, :, era5_time_idx)); % lon×lat
    
    % 提取CCKW散度异常 (已排序)
    div_kelvin_slice = squeeze(kelvin_div_sorted(:, :, kelvin_time_idx)); % lon×lat
    ...
    % 插值CCKW散度到统一网格
    div_kelvin_interp = interp2(kelvin_lon_sorted, kelvin_lat_sorted, div_kelvin_slice', LON, LAT, 'linear', 0);
    ...
    % 计算涡旋拉伸项: abs_vorticity * div_kelvin
    % 注意: Python代码中使用的是 -div_kelvin，我们需要检查符号
    vortex_stretching = abs_vorticity .* (-div_kelvin_interp);
...
for i = 1:length(valid_dates)
    ...
    % 添加标题
    title_str = sprintf('Vortex Stretching: CCKW Divergence × Absolute Vorticity\nDate: %s UTC | TC Freddy: %.2f°E, %.2f°S', ...
...
fprintf(info_file, '热带气旋Freddy生成期间CCKW对涡旋拉伸贡献的三维立体图\n');
...
fprintf(info_file, '  3. 从滤波数据提取CCKW散度异常\n');
fprintf(info_file, '  4. 计算涡旋拉伸: ζ_abs × div_CCKW\n');
