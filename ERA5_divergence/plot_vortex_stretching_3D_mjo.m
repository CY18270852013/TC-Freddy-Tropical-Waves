...
% 输出设置
output_dir = 'mjo_vortex_stretching_3d';
...
% 读取MJO散度数据
mjo_time_raw = ncread(mjo_div_file, 'time'); % days since 0000-01-01
mjo_lat = ncread(mjo_div_file, 'latitude');
mjo_lon = ncread(mjo_div_file, 'longitude');
mjo_div = ncread(mjo_div_file, 'divergence_anomaly_mjo'); % s^-1

% 转换时间
base_date2 = datetime(0, 1, 1, 'TimeZone', 'UTC');
mjo_time = base_date2 + days(mjo_time_raw) - days(1);

fprintf('MJO数据维度: 时间=%d, 纬度=%d, 经度=%d\n', ...
    length(mjo_time), length(mjo_lat), length(mjo_lon));

% 同样处理MJO数据的网格
mjo_lat_sorted = flipud(mjo_lat); % 从-30到10

% 调整经度格式并排序
mjo_lon_adj = mjo_lon;
mjo_lon_adj(mjo_lon_adj > 180) = mjo_lon_adj(mjo_lon_adj > 180) - 360;
[mjo_lon_sorted, mjo_lon_sort_idx] = sort(mjo_lon_adj);

% 重新排列mjo_div数据
mjo_div_sorted = zeros(size(mjo_div));
for t = 1:size(mjo_div, 3)
    temp = flip(mjo_div(:, :, t), 2); % 反转纬度维度
    mjo_div_sorted(:, :, t) = temp(mjo_lon_sort_idx, :);
end
...
for t = 1:length(target_times)
    ...
    % 从MJO数据中找到最接近的时间索引
    [~, mjo_time_idx] = min(abs(mjo_time - target_time));
    mjo_actual_time = mjo_time(mjo_time_idx);
    
    fprintf('  ERA5实际时间: %s (差%.1f小时)\n', datestr(era5_actual_time), ...
        hours(abs(era5_actual_time - target_time)));
    fprintf('  MJO实际时间: %s (差%.1f小时)\n', datestr(mjo_actual_time), ...
        hours(abs(mjo_actual_time - target_time)));
    
    % 提取850 hPa相对涡度 (已排序)
    vo_850_slice = squeeze(vo_850_sorted(:, :, era5_time_idx)); % lon×lat
    
    % 提取MJO散度异常 (已排序)
    div_mjo_slice = squeeze(mjo_div_sorted(:, :, mjo_time_idx)); % lon×lat
    ...
    % 插值MJO散度到统一网格
    div_mjo_interp = interp2(mjo_lon_sorted, mjo_lat_sorted, div_mjo_slice', LON, LAT, 'linear', 0);
    ...
    % 计算涡旋拉伸项: abs_vorticity * div_mjo
    vortex_stretching = abs_vorticity .* (-div_mjo_interp);
...
for i = 1:length(valid_dates)
    ...
    % 添加标题
    title_str = sprintf('Vortex Stretching: MJO Divergence × Absolute Vorticity\nDate: %s UTC | TC Freddy: %.2f°E, %.2f°S', ...
...
fprintf(info_file, '热带气旋Freddy生成期间MJO对涡旋拉伸贡献的三维立体图\n');
...
fprintf(info_file, '  3. 从滤波数据提取MJO散度异常\n');
fprintf(info_file, '  4. 计算涡旋拉伸: ζ_abs × div_MJO\n');
