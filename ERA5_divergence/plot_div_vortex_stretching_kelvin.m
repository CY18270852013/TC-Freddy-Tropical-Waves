%% 绘制Freddy genesis box的CCKW散度和涡旋拉伸项时间序列图
clear; clc; close all;
...
% 数据文件路径
cckw_div_file = "E:\Results_of_Freddy\divergence_anomaly_6hourly\divergence_anomaly_for_Kelvin_6hourly_20220101_20231231.nc";
dv_files = {
...
%% 2. 读取CCKW散度异常数据
fprintf('正在读取CCKW散度异常数据...\n');

% 读取CCKW数据
cckw_lat = ncread(cckw_div_file, 'latitude');
cckw_lon = ncread(cckw_div_file, 'longitude');
cckw_time = ncread(cckw_div_file, 'time');
cckw_div_anom = ncread(cckw_div_file, 'divergence_anomaly_kelvin');

% 将时间转换为datetime格式 (days since 0000-01-01 00:00:00)
% 注意: MATLAB的datetime从0000年1月1日开始
cckw_datetime = datetime(0, 1, 1) + days(cckw_time) - days(1);

% 提取2023年2月1日至2月12日的数据
time_mask = (cckw_datetime >= start_date) & (cckw_datetime <= end_date);
cckw_time_subset = cckw_time(time_mask);
cckw_datetime_subset = cckw_datetime(time_mask);
cckw_div_anom_subset = cckw_div_anom(:, :, time_mask);

% 查找在目标区域的索引
lon_idx = find(cckw_lon >= lon_min & cckw_lon <= lon_max);
lat_idx = find(cckw_lat >= lat_min & cckw_lat <= lat_max);

% 提取目标区域的数据
cckw_div_box = cckw_div_anom_subset(lon_idx, lat_idx, :);

% 计算区域平均值 (空间平均)
cckw_div_mean = squeeze(mean(mean(cckw_div_box, 1), 2));

fprintf('CCKW数据读取完成，时间点数量: %d\n', length(cckw_datetime_subset));
...
common_times = intersect(cckw_datetime_subset, all_times);
...
% 提取共同时间点的数据
cckw_div_mean_aligned = zeros(length(common_times), 1);
abs_vort_mean_aligned = zeros(length(common_times), 1);

for i = 1:length(common_times)
    % CCKW数据
    cckw_idx = find(cckw_datetime_subset == common_times(i));
    if ~isempty(cckw_idx)
        cckw_div_mean_aligned(i) = cckw_div_mean(cckw_idx);
    end
...
% 计算涡旋拉伸项: η × (-∇·V')
% vortex stretching = 绝对涡度 × (-CCKW散度)
vortex_stretching = abs_vort_mean_aligned .* (-cckw_div_mean_aligned);  % s^-2

%% 5. 数据范围分析和y轴设置
% 分析数据范围以确定合适的y轴刻度
cckw_range = [min(cckw_div_mean_aligned), max(cckw_div_mean_aligned)];
vortex_range = [min(vortex_stretching), max(vortex_stretching)];

fprintf('\n数据范围分析:\n');
fprintf('CCKW散度异常范围: [%.3e, %.3e] s^-1\n', cckw_range(1), cckw_range(2));
fprintf('涡旋拉伸项范围: [%.3e, %.3e] s^-2\n', vortex_range(1), vortex_range(2));

% 计算合适的y轴范围（增加10%的边距）
cckw_ymin = min(cckw_div_mean_aligned) * 1e6 - 0.2;
cckw_ymax = max(cckw_div_mean_aligned) * 1e6 + 0.2;
...
% 确保y轴范围对称，使零线居中
cckw_abs_max = max(abs([cckw_ymin, cckw_ymax]));
cckw_ymin = -cckw_abs_max;
cckw_ymax = cckw_abs_max;
...
% 绘制双y轴图
yyaxis left;
plot(common_times, cckw_div_mean_aligned * 1e6, '-', 'Color', [0.1, 0.3, 0.8], 'LineWidth', 2);  % 转换为10^-6 s^-1
ylabel('CCKW Low-Level Divergence Anomaly (10^{-6} s^{-1})', 'FontSize', 12, 'FontWeight', 'bold');
ylim([cckw_ymin, cckw_ymax]);  % 使用计算的范围
grid on;
...
% 设置标题
title('CCKW Influence on Vorticity in Freddy Genesis Box (110-120°E, 10-20°S)', ...
    'FontSize', 14, 'FontWeight', 'bold');

% 添加图例
legend({'CCKW Divergence Anomaly (850 hPa)', 'Vortex Stretching Term'}, ...
...
%% 7. 保存图像
output_filename = 'Freddy_Genesis_Box_CCKW_Vortex_Stretching.png';
...
fprintf('时间点数量: %d\n', length(common_times));
fprintf('CCKW散度异常范围: [%.3e, %.3e] s^-1\n', min(cckw_div_mean_aligned), max(cckw_div_mean_aligned));
fprintf('涡旋拉伸项范围: [%.3e, %.3e] s^-2\n', min(vortex_stretching), max(vortex_stretching));
fprintf('平均绝对涡度: %.3e s^-1\n', mean(abs_vort_mean_aligned));

% 计算并显示峰值信息
[cckw_max, cckw_max_idx] = max(abs(cckw_div_mean_aligned));
[vortex_max, vortex_max_idx] = max(abs(vortex_stretching));
fprintf('CCKW散度异常最大绝对值: %.3e s^-1 出现在 %s\n', cckw_max, datestr(common_times(cckw_max_idx)));
...
subplot(1, 3, 1);
plot(common_times, cckw_div_mean_aligned * 1e6, 'b-', 'LineWidth', 1.5);
xlabel('Date');
ylabel('CCKW Div Anom (10^{-6} s^{-1})');
title('CCKW Divergence Anomaly');
datetick('x', 'mm/dd', 'keeplimits');
% xtickangle(45);
grid on;
...
% 保存第二个图
print(gcf, 'Kelvin_Freddy_Individual_Time_Series.png', '-dpng', '-r300');
