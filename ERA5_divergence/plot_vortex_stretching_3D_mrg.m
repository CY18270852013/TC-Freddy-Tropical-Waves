...
% 输出设置
output_dir = 'mrg_vortex_stretching_3d';
...
% 读取MRG散度数据
mrg_time_raw = ncread(mrg_div_file, 'time'); % days since 0000-01-01
mrg_lat = ncread(mrg_div_file, 'latitude');
mrg_lon = ncread(mrg_div_file, 'longitude');
mrg_div = ncread(mrg_div_file, 'divergence_anomaly_mrg'); % s^-1

% 转换时间
base_date2 = datetime(0, 1, 1, 'TimeZone', 'UTC');
mrg_time = base_date2 + days(mrg_time_raw) - days(1);

fprintf('MRG数据维度: 时间=%d, 纬度=%d, 经度=%d\n', ...
    length(mrg_time), length(mrg_lat), length(mrg_lon));

% 同样处理MRG数据的网格
mrg_lat_sorted = flipud(mrg_lat); % 从-30到10

% 调整经度格式并排序
mrg_lon_adj = mrg_lon;
mrg_lon_adj(mrg_lon_adj > 180) = mrg_lon_adj(mrg_lon_adj > 180) - 360;
[mrg_lon_sorted, mrg_lon_sort_idx] = sort(mrg_lon_adj);

% 重新排列mrg_div数据
mrg_div_sorted = zeros(size(mrg_div));
for t = 1:size(mrg_div, 3)
    temp = flip(mrg_div(:, :, t), 2); % 反转纬度维度
    mrg_div_sorted(:, :, t) = temp(mrg_lon_sort_idx, :);
end
...
for t = 1:length(target_times)
    ...
    % 从MRG数据中找到最接近的时间索引
    [~, mrg_time_idx] = min(abs(mrg_time - target_time));
    mrg_actual_time = mrg_time(mrg_time_idx);
    
    fprintf('  ERA5实际时间: %s (差%.1f小时)\n', datestr(era5_actual_time), ...
        hours(abs(era5_actual_time - target_time)));
    fprintf('  MRG实际时间: %s (差%.1f小时)\n', datestr(mrg_actual_time), ...
        hours(abs(mrg_actual_time - target_time)));
    
    % 提取850 hPa相对涡度 (已排序)
    vo_850_slice = squeeze(vo_850_sorted(:, :, era5_time_idx)); % lon×lat
    
    % 提取MRG散度异常 (已排序)
    div_mrg_slice = squeeze(mrg_div_sorted(:, :, mrg_time_idx)); % lon×lat
    ...
    % 插值MRG散度到统一网格
    div_mrg_interp = interp2(mrg_lon_sorted, mrg_lat_sorted, div_mrg_slice', LON, LAT, 'linear', 0);
    ...
    % 计算涡旋拉伸项: abs_vorticity * div_mrg
    vortex_stretching = abs_vorticity .* (-div_mrg_interp);
...
for i = 1:length(valid_dates)
    ...
    % 添加标题
    title_str = sprintf('Vortex Stretching: MRG Divergence × Absolute Vorticity\nDate: %s UTC | TC Freddy: %.2f°E, %.2f°S', ...
...
fprintf(info_file, '热带气旋Freddy生成期间MRG对涡旋拉伸贡献的三维立体图\n');
...
fprintf(info_file, '  3. 从滤波数据提取MRG散度异常\n');
fprintf(info_file, '  4. 计算涡旋拉伸: ζ_abs × div_MRG\n');
