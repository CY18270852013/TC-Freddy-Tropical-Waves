clear;clc;
filename = 'E:\科研\热带气旋Freddy极端长生命史原因探究\Processed_Data_of_Freddy\GPM_anomaly_daily\GPM_anomaly_for_ER_daily_20220901_20230630.nc';

lon = ncread(filename, 'lon');
lat = ncread(filename, 'lat');
time = ncread(filename, 'time');
er_data = ncread(filename, 'er'); % 维度: lon x lat x time
...
subset_data = er_data(lon_idx, lat_idx, time_idx);
% 计算纬度平均（10°S~10°N）
...
title('Time-Longitude ER Hovmöller Diagram: Precipitation Anomaly (10°S-10°N)', 'FontSize', 14);
...
set(gca, 'FontSize', 11);
set(gca, 'Layer', 'top');
saveas(gcf, 'er_hovmoller_diagram_10S_10N.png', 'png');
fprintf('绘图完成！\n');